...
namespace GMEPSolar
{
  public partial class PowerStationUserControl : UserControl
  {
    ...
    private void MakeSingleLine(Point3d startingPoint)
    {
      int kwId = PowerStation.KwId;
      ...
      string panelBlock = "IBEC 100A PANEL";
      string wireSize = "3WG100";
      Point3d meterComboPoint = new Point3d(startingPoint.X + 6.4373, startingPoint.Y - 4.2534, 0);
      Point3d batteryPoint = new Point3d(startingPoint.X + 5.1288, startingPoint.Y - 4.0957, 0);
      Point3d batteryLinePoint = new Point3d(
        startingPoint.X + 5.1288,
        startingPoint.Y - 4.0957 - 0.4748,
        0
      );
      string powerStationBlock = "IBEC 30KW POWER STATION";
      if (kwId == 2)
      {
        panelPoint = new Point3d(
        ...
        panelBlock = "IBEC 200A PANEL";
        wireSize = "3WG200";
        meterComboPoint = new Point3d(startingPoint.X + 8.0855, startingPoint.Y - 4.2534, 0);
        batteryPoint = new Point3d(startingPoint.X + 6.7770, startingPoint.Y - 4.0957, 0);
        batteryLinePoint = new Point3d(
          startingPoint.X + 6.7770,
          startingPoint.Y - 4.0957 - 0.4748,
          0
        );
        powerStationBlock = "IBEC 45KW POWER STATION";
      }
      List<Lot> ActiveLots = GetActiveLots();

      if (ActiveLots.Count > 1)
      {
        CadObjectFunctions.MakeBlock(panelPoint, panelBlock);
      ...
      {
        Point3d currentMeterComboPoint = new Point3d(
          meterComboPoint.X + meterComboPointXOffset,
          meterComboPoint.Y,
          0
        );
        meterComboPoints.Add(currentMeterComboPoint);
        if (ActiveLots.Count > 1)
        {
          ...
          Point3d intersectionPoint = new Point3d(currentMeterComboPoint.X, panelBreakerPoint.Y, 0);
          CadObjectFunctions.MakeLine(panelBreakerPoint, intersectionPoint);
          CadObjectFunctions.MakeLine(intersectionPoint, currentMeterComboPoint);
        }
        CadObjectFunctions.MakeText(
          new Point3d(currentMeterComboPoint.X + 0.2076, currentMeterComboPoint.Y + 4.928, 0),
          "(E)METER COMBINATION AT LOT " + ActiveLots[i].Number
        );
        CadObjectFunctions.MakeText(
          new Point3d(currentMeterComboPoint.X + 0.2076, currentMeterComboPoint.Y + 4.7373, 0),
          $"{ActiveLots[i].Voltage}, {ActiveLots[i].Amp}A"
        );
        CadObjectFunctions.MakeText(
          new Point3d(currentMeterComboPoint.X + 0.2076, currentMeterComboPoint.Y + 4.5823, 0),
          ActiveLots[i].Kaic + " KAIC"
        );
        CadObjectFunctions.MakeText(
          new Point3d(currentMeterComboPoint.X + 1.3979, currentMeterComboPoint.Y + 2.4252, 0),
          $"{ActiveLots[i].Amp}A/" + (ActiveLots[i].Voltage.Contains("4W") ? "3P" : "2P")
        );
        if (i == ActiveLots.Count - 1)
        {
          CadObjectFunctions.MakeBlock(
            new Point3d(currentMeterComboPoint.X + 1.1472, currentMeterComboPoint.Y + 1.3900, 0),
            "IBEC 60A-2P TEMP POWER BREAKER"
          );
          if (ActiveLots.Count > 1)
          {
            CadObjectFunctions.MakeText(
              new Point3d(currentMeterComboPoint.X + 2.6945, currentMeterComboPoint.Y + 0.2734, 0),
              "(TYP.)"
            );
          }
        }
      ...
      {
        batteryPoints.Add(new Point3d(batteryPoint.X, batteryPoint.Y + batteryPointYOffset, 0));
        if (i < PowerStation.NumBatts - 1)
        {
          Point3d line1Start = new Point3d(
            batteryLinePoint.X,
            batteryLinePoint.Y + batteryPointYOffset,
            0
          );
      ...
      CadObjectFunctions.MakeBlock(startingPoint, powerStationBlock);
